...
class GridPositionGoalWrapper(gym.Wrapper):
    ...
    def step(self, action: Any) -> Tuple[Any, SupportsFloat, bool, bool, Dict[str, Any]]:
        ...
        if abstract_state != self.prev_state:
            ...
            reward = -GridAbstraction.grid_distance_from_state(self.prev_goal.walker_pos, curr_state)

        info['goal'] = new_goal
        info['achieved_subgoal'] = abstract_state == self.prev_goal
        info['new_abstract_state'] = abstract_state != self.prev_state
        ...
        return np.concatenate([obs, self.goal_obs_func(new_goal)]), reward, term, trunc, info
