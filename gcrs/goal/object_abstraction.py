...
def goal_task_getter(obs: Observation, abstract: ObjectAbstraction, env: gym.Env):
    ...
    def eval_state(abstract: ObjectAbstraction):
        for entity_a, entity_b in abstract.nears:
            if (
                entity_a == 0
                and abstract.objects[entity_b - 1][0] == ObjectAbstraction.GOAL_IDX
            ):
                return 1, True
        return 0, False
...
def pickup_object_task_getter(
    obs: Observation, abstract: ObjectAbstraction, env: gym.Env, strict=False
):
...
def pickup_object_strict_task_getter(
    obs: Observation, abstract: ObjectAbstraction, env: gym.Env
):
...
def object_obs_fn(abstract: ObjectAbstraction, env=None):
    """Observe the ObjectAbstraction to get information about held object and nearby object."""
...
def object_obs_fn_one_hot(abstract: ObjectAbstraction):
    """Observe the ObjectAbstraction to get information about held object and nearby object, encoded as one-hot."""
    held_idx = abstract.get_held_object()
    if held_idx == -1:
        held_type, held_color = -1, -1
    else:
...
def get_object_abstraction_goal_wrapper(
    env: gym.Env,
    env_id: str,
